...
export class AuthorizationController {
    ...
    async AuthRedirect(@Query('code') code, @Res() res) {
        ...
        if (user) {
            if (!user.otp_enabled)
                res.redirect(`http://${process.env.BACKEND_IP}:8081/login?token=${user.token}`);
            else {
                res.redirect(`http://${process.env.BACKEND_IP}:8081/2fa?userId=${user.id}`);
            }
        }
        else res.redirect(`http://${process.env.BACKEND_IP}:8081/login?token=null`);
    }
